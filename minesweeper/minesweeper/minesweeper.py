...
PERKS = {
    "Iron Will":    {"desc": "Gain +1 extra life.",                                    "stackable": False},
    "Eagle Eye":    {"desc": "Auto-reveals +1 safe cell per stack at floor start.",    "stackable": True},
    "Cartographer": {"desc": "Corner cells are revealed at floor start.",              "stackable": False},
    "Scavenger":    {"desc": "Each stack adds +3 XP per correct flag.",               "stackable": True},
    "Berserker":    {"desc": "Chord reveals +1 extra safe cell per stack.",            "stackable": True},
    "Sixth Sense":  {"desc": "Flashes +1 mine location per stack at floor start.",    "stackable": True},
    "Lucky Charm":  {"desc": "Each stack adds +10% chance for mines to be duds.",     "stackable": True},
    "Pathfinder":   {"desc": "First click always opens a large clear area.",          "stackable": False},
}

# Theme fields: (name, bg, fg, icon, cell_unrevealed, cell_revealed, flag_char, mine_char, number_colors)
FLOOR_THEMES = [
    ("Dungeon",  "#1a1a2e", "#e8c97a", "💀",
     "#2a2a44", "#13132a", "🚩", "💣",
     {1:"#6ec6ff",2:"#81c784",3:"#e57373",4:"#9575cd",5:"#ff8a65",6:"#4dd0e1",7:"#f06292",8:"#eeeeee"}),

    ("Crypt",    "#0d1b2a", "#9fbbcc", "🕯️",
     "#1a2e3a", "#0a1a24", "🕯️", "💀",
     {1:"#aed6f1",2:"#a9dfbf",3:"#f1948a",4:"#bb8fce",5:"#f0b27a",6:"#76d7c4",7:"#f1948a",8:"#d5d8dc"}),

    ("Cavern",   "#1c1c1e", "#b87333", "⛏️",
     "#2e2416", "#1a1510", "⛏️", "💥",
     {1:"#e59866",2:"#a9cce3",3:"#ec7063",4:"#a569bd",5:"#f0b27a",6:"#45b39d",7:"#eb984e",8:"#ccd1d1"}),

    ("Abyss",    "#0a0a14", "#7b68ee", "🌀",
     "#16163a", "#0a0a22", "🔮", "🌀",
     {1:"#a29bfe",2:"#55efc4",3:"#fd79a8",4:"#6c5ce7",5:"#fdcb6e",6:"#00cec9",7:"#e17055",8:"#dfe6e9"}),

    ("Hellgate", "#1a0000", "#ff6b6b", "🔥",
     "#3a0a0a", "#220404", "🔥", "💣",
     {1:"#ff7675",2:"#fdcb6e",3:"#d63031",4:"#e17055",5:"#fab1a0",6:"#ff7043",7:"#e53935",8:"#ffccbc"}),

    ("Void",     "#050510", "#00ffcc", "⭐",
     "#0a1a2a", "#030310", "✨", "⭐",
     {1:"#00ffcc",2:"#00b4d8",3:"#7209b7",4:"#3a0ca3",5:"#f72585",6:"#4cc9f0",7:"#560bad",8:"#ffffff"}),
]
...
class RogueMinesweeper:
    ...
    def show_perk_screen(self, callback):
        ...
        # Pick 3 choices — stackable perks can appear even if already owned
        non_stackable_owned = [p for p in self.perks if not PERKS[p]["stackable"]]
        available_stackable = [p for p in PERKS if PERKS[p]["stackable"]]
        available_new       = [p for p in PERKS if not PERKS[p]["stackable"] and p not in self.perks]
        pool = available_stackable + available_new
        if not pool:
            pool = available_stackable  # fallback: only stackables left
        choices = random.sample(pool, min(3, len(pool)))
        if not choices:
            tk.Label(wrap, text="🏆  ALL PERKS MASTERED",
                     font=("Courier New", 13, "bold"),
                     bg=bg, fg="#81c784").pack(pady=(0, 10))
            tk.Label(wrap, text="You've collected every perk.\nYour power is complete.",
                     font=("Courier New", 10), bg=bg, fg="#aaaacc",
                     justify="center").pack(pady=(0, 20))
            tk.Button(wrap, text="Continue →",
                      font=("Courier New", 11, "bold"),
                      bg="#e8c97a", fg="#0d0d1a", activebackground="#ffe08a",
                      relief=tk.FLAT, padx=16, pady=8, cursor="hand2",
                      command=lambda: (self._clear_window(), callback())).pack()
            return

        for perk in choices:
            stack = self.perks.count(perk)
            pf = tk.Frame(wrap, bg="#1a1a30", bd=0, relief=tk.FLAT)
            pf.pack(fill=tk.X, padx=40, pady=5, ipadx=10, ipady=8)
            label = f"  {perk}"
            if stack > 0:
                label += f"  ×{stack + 1}" if PERKS[perk]["stackable"] else "  ✓"
            tk.Label(pf, text=label,
                     font=("Courier New", 12, "bold"),
                     bg="#1a1a30", fg="#e8c97a", anchor="w").pack(fill=tk.X)
            tk.Label(pf, text=f"  {PERKS[perk]['desc']}",
                     font=("Courier New", 9),
    ...
    def _build_floor(self, rows, cols, mines, theme):
        ...
        self.theme       = theme
        (_, self.theme_bg, self.theme_fg, self.theme_icon,
         self.theme_cell, self.theme_revealed,
         self.theme_flag, self.theme_mine,
         self.theme_numbers) = theme
        ...
        if self.perks:
            ...
            for p, count in seen.items():
                short = p[:3].upper()
                label = f"[{short}×{count}]" if count > 1 else f"[{short}]"
                tk.Label(perk_bar, text=label,
                         font=("Courier New", 8),
        ...
        for r in range(self.rows):
            ...
            for c in range(self.cols):
                btn = tk.Button(
                    ...
                    bg=self.theme_cell, fg=self.theme_fg,
                    activebackground=self.theme_revealed,
    ...
    def place_mines(self, safe_r, safe_c):
        ...
        # Lucky Charm: each stack adds 10% dud chance, capped at 60%
        lucky_stacks = self.perks.count("Lucky Charm")
        if lucky_stacks > 0:
            dud_chance = min(0.10 * lucky_stacks, 0.60)
            dud_count = max(1, int(len(self.mine_locs) * dud_chance))
    ...
    def _gain_xp(self, amount):
        ...
        return levels_gained  # 0 = no level-up, >1 = multiple
    ...
    def click(self, r, c):
        ...
        self.reveal(r, c)
        cells_revealed = len(self.revealed) - before
        levels_gained = self._gain_xp(XP_PER_SAFE * cells_revealed)
        self.score += 1
        ...
        if floor_done:
            ...
            self._pending_levelups = levels_gained
            self.master.after(200, self._show_next_levelup_resume)

    def _show_next_levelup_resume(self):
        """Show one perk screen for a mid-floor level-up, then chain to the next or resume."""
        self._pending_levelups -= 1
        remaining = self._pending_levelups
        if remaining > 0:
            callback = self._show_next_levelup_resume
        else:
            callback = self._resume_floor
        self.show_perk_screen(callback)

    def _show_next_levelup_descend(self):
        """Show one perk screen between floors, then chain to the next or descend."""
        self._pending_levelups -= 1
        remaining = self._pending_levelups
        if remaining > 0:
            callback = self._show_next_levelup_descend
        else:
            callback = self.next_floor
        self.show_perk_screen(callback)
    ...
    def _resume_floor(self):
        ...
        for (r, c) in saved_revealed:
            ...
            btn.config(relief=tk.SUNKEN,
                       bg=self.theme_revealed if not is_dud else self.theme_cell,
                       state=tk.DISABLED, text="")
            if is_dud:
                btn.config(text="💫", fg="#ffe08a")
            elif count > 0:
                btn.config(text=str(count), fg=self.theme_numbers.get(count, "#eeeeee"))

        for (r, c) in saved_flagged:
            if (r, c) not in self.revealed:
                self.flagged.add((r, c))
                self.buttons[r][c].config(text=self.theme_flag, bg=self.theme_revealed)
    ...
    def _post_first_click_perks(self, r, c):
        ...
        if getattr(self, "_sixth_sense_pending", False):
            count = self.perks.count("Sixth Sense")
            candidates = list(self.mine_locs)
            shown = random.sample(candidates, min(count, len(candidates)))
            for m in shown:
                self.buttons[m[0]][m[1]].config(bg="#ff4444", text=self.theme_mine)
                self.master.after(1200, lambda m=m: self._hide_sixth_sense(m))
            self._sixth_sense_pending = False

        eagle_stacks = self.perks.count("Eagle Eye")
        if eagle_stacks > 0:
            safes = [(r2, c2) for r2 in range(self.rows) for c2 in range(self.cols)
                     ...
                     and (r2, c2) != (r, c)]
            picks = random.sample(safes, min(eagle_stacks, len(safes)))
            for pick in picks:
    ...
    def _hide_sixth_sense(self, m):
        if m not in self.revealed and m not in self.flagged:
            self.buttons[m[0]][m[1]].config(bg=self.theme_cell, text="")
    ...
    def toggle_flag(self, r, c):
        ...
        if (r, c) in self.flagged:
            ...
            self.buttons[r][c].config(text="", bg=self.theme_cell)
        else:
            self.flagged.add((r, c))
            self.buttons[r][c].config(text=self.theme_flag, bg=self.theme_revealed)
            bonus_xp = XP_PER_FLAG * self.perks.count("Scavenger")
            # Only give XP if flagging an actual mine
            if (r, c) in self.mine_locs:
                levels_gained = self._gain_xp(XP_PER_FLAG + bonus_xp)
                self.total_flags += 1
                if levels_gained > 0 and not self.floor_ended:
                    self.floor_ended = True
                    self._pending_levelups = levels_gained
                    self.master.after(200, self._show_next_levelup_resume)
        remaining = self.mine_count - len(self.flagged)
    ...
    def chord_click(self, r, c):
        ...
        if flags == count:
            ...
            for nr, nc in targets:
                self.click(nr, nc)
            # Berserker: reveal N extra safe cells from the outer ring (N = stack count)
            berserker_stacks = self.perks.count("Berserker")
            if berserker_stacks > 0:
                ring = set()
                for nr, nc in self.get_neighbors(r, c):
                    for nr2, nc2 in self.get_neighbors(nr, nc):
                        if (nr2, nc2) not in self.flagged and (nr2, nc2) not in self.revealed and (nr2, nc2) != (r, c):
                            ring.add((nr2, nc2))
                safe_ring = [cell for cell in ring if cell not in self.mine_locs or cell in self.duds]
                picks = random.sample(safe_ring, min(berserker_stacks, len(safe_ring)))
                for cell in picks:
                    self.click(*cell)
        ...
        while queue:
            ...
            if is_dud:
                ...
                btn.config(text=str(count),
                           fg=self.theme_numbers.get(count, "#eeeeee"))
            else:
                for nr, nc in self.get_neighbors(r, c):
        ...
        for mr, mc in self.mine_locs:
            if (mr, mc) not in self.revealed and (mr, mc) not in self.duds:
                self.buttons[mr][mc].config(
                    text=self.theme_mine, bg="#440000" if (mr, mc) != (r, c) else "#ff2200",
                    state=tk.DISABLED
                )
        self.lives -= 1
    ...
    def _floor_clear(self):
        ...
        levels_gained = self._gain_xp(bonus)
        self.score += bonus
        ...
        for mr, mc in self.mine_locs:
            if (mr, mc) not in self.flagged and (mr, mc) not in self.duds:
                ...
                self.buttons[mr][mc].config(text=self.theme_flag, bg="#002200")

        if levels_gained > 0:
            self._pending_levelups = levels_gained
            self.master.after(600, self._show_next_levelup_descend)
        else:
